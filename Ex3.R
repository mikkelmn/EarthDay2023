...
data = nc_open(filename = "temp_anomaly_ex3.nc")
...
input = map(.x = seq(100, 142000, 100), 
            .f = function(x) input[(x-99):x])

input = map(.x = 1:1420, 
            .f = function(x) input[[x]][-c(45:47, 55:57, 65:67)]) %>%
  flatten_dbl() %>% matrix(ncol = 1420)
...
model %>% layer_dense(units = 50, activation = "relu", input_shape = c(91)) %>% 
...
write.csv(x = pred, file = "pred_Ex3_MMN")
