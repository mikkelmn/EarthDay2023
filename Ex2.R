...
data = nc_open(filename = "temp_anomaly_ex2.nc")
...
input = map(.x = 1:1420, 
            .f = function(x) input[[x]][-c(55, 56, 65, 66)]) %>% flatten_dbl() %>% matrix(ncol = 1420)
...
model %>% layer_dense(units = 50, activation = "relu", input_shape = c(96)) %>% 
...
write.csv(x = pred, file = "pred_Ex2_MMN")
